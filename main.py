...
# cv2.CHAIN_APPROX_NONE : 輪郭上の全画素(点)の情報を利用し、輪郭の検出を行う。画素数が多い場合に、メモリの使用量が大きくなり、重くなるので注意が必要。
####################################################
# 戻り値  ###########################################
# image : 輪郭付き画像情報
# contours : 輪郭になりうる画素(点)情報
# hierarchy : 輪郭の階層構造
####################################################
...
# 第一引数 : 画像情報
# 第二引数 : 輪郭になりうる画素(点)情報
# 第三引数 : 輪郭になりうる画素(点)のインデックス番号を指定する。例えば0を指定すると、1番目の輪郭になりうる画素(点)のみを描画する。1を指定すると、2番目の輪郭になりうる画素(点)のみを描画する。輪郭になりうる画素(点)を全て描画したい場合は、-1を指定する。
# 第四引数 : 輪郭になりうる画素(点)の色。RGB指定。
# RGBとは? : https://ja.wikipedia.org/wiki/RGB
# 第五引数(任意) : 輪郭になりうる画素(点)の大きさを設定。デフォルト1。
# 戻り値 : 画像情報
